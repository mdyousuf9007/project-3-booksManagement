  ...
  try {
    ...
    for (field of requiredFields) {
      if (!isValid(req.body[field])) {
        return res
          .status(400)
          .send({ status: false, msg: `this ${field} key is invalid` });
      }
    }
    ...
    if (!book) {
      return res
        .status(404)
        .send({ status: false, msg: "Book is not available for this ID" });
    } else {
      data.bookId = bookId;
    }
  ...
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(reviewId)) {
      return res
        ...
        .send({
          status: false,
          msg: "reviewId is not valid,please enter valid ID",
        });
    }
    ...
    if (!review) {
      return res
        .status(404)
        .send({ status: false, msg: "review is not found for this ID" });
    }
    ...
    if (review.isDeleted === true) {
        return res
          .status(400)
          .send({ status: false, msg: "this review Document is already deleted" });
      }
